            ...
            if ($webspace) { // use html-friendly output
                ...
                while (list($key, $value) = each($data))
                    echo htmlspecialchars($key) . " = " . htmlspecialchars($value) . "<BR>\n";
							...
							echo __('Debugging / Sandbox output', 'event_espresso');
...
# TRANSACT #
                ...
                if ($this->debugging) {
                    if ($this->webspace)
                        echo "using BINARY curl methods<br>server responds:<br>" . htmlspecialchars($result) . "<br><br>";
                    else      // non html output
                        echo "using BINARY curl methods\nserver responds:\n $result\n\n";
                }
            ...
            $ch = curl_init();
            $error=array();
            $error["set_host"]=curl_setopt($ch, CURLOPT_URL, $host);
            $error["set_post"]=curl_setopt($ch, CURLOPT_POST, 1);
            $error["set_post_fields"]=curl_setopt($ch, CURLOPT_POSTFIELDS, $xml);
            $error["set_ssl_cert"]=curl_setopt($ch, CURLOPT_SSLCERT, $key);
						if (!$data['verify_peer']) {
							curl_setopt ($ch, CURLOPT_SSL_VERIFYHOST, 0);
							curl_setopt ($ch, CURLOPT_SSL_VERIFYPEER, 0);
						}
            $error["set_return_transfer"]=curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);

            if ($this->debugging) {
                echo "<br>Using BUILT-IN PHP curl methods<br>";
                echo $error["set_host"] ? "Success" : "Failure";
                echo " Setting host: ".$host."<br>";
                echo $error["set_post"] ? "Success" : "Failure";
                echo " Setting request type to post<br>";
                echo $error["set_post_fields"] ? "Success" : "Failure";
                echo " Setting post fields: ".htmlspecialchars($xml)."<br>";
                echo $error["set_ssl_cert"] ? "Success" : "Failure";
                echo " Setting SSL certificate key: ".$key."<br>";
		echo $error["set_return_transfer"] ? "Success" : "Failure";
                echo " Setting return transfer<br>";
                $error["set_verbose"]=curl_setopt($ch, CURLOPT_VERBOSE, 1);
                echo $error["set_verbose"] ? "Success" : "Failure";
                echo " Setting verbose mode<br>";
            }
            ...
            if ($this->debugging) {
                echo "Result of executing curl transfer: ".$error["result"];
...
//			 echo htmlspecialchars($key) . " = " . htmlspecialchars($value) . "<br>\n";
        ...
        if ($this->debugging) { // make it easy to see      // LSGS doesn't mind whitespace
            reset($pdata);
