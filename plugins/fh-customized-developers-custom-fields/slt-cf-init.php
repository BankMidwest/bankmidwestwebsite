...
function slt_cf_init() {
	...
	wp_register_script( 'perifer-timePicker', plugins_url('js/perifer/jquery.timePicker.js', __FILE__ ), array(  ), '0.1', true );
	wp_register_style( 'perfier-timepicker-css', plugins_url('js/perifer/timePicker.css', __FILE__ ));
...
function slt_cf_admin_init() {
	...
	// Datepicker
	if(!isset($_GET['page']) || !in_array($_GET['page'], array("gf_new_form", "gf_export")))
    {
	...
	// Perifer datetimepicker
	wp_enqueue_script('perifer-timePicker');
	wp_enqueue_style('perfier-timepicker-css');
