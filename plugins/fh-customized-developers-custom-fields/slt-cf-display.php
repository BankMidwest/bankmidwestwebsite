...
function fh_format_jquery_date($timestamp, $format)
{
	// Our "database" of timestamp conversions.  Probably a cleaner way, but we'll prescribe possibilities here.
	$conversion = "";
	switch($format)
	{
		case "mm/dd/yy":
	...
	// Timestamp is expected as Unix timestamp numeric (string or int).
	if(is_numeric($timestamp))
	{
		// We're expecting a ms-based epoch differential, not a PHP-based seconds one...
		$timestamp = preg_replace("/(\d+)000$/", "$1", $timestamp);
		return date($conversion, intval($timestamp));
	}
	return "";	// unknown timestamp ends up here.  don't know what to return for now... (TODO: reimplement)
}
		...
		switch ( $field['type'] ) {
			...
			case 'time': {
				...
				*****************************************************************/
				...
				{
					$dt = DateTime::createFromFormat("U", doubleval($field_value));
					$field_value_disp = $dt->format("H:i A");
				}
				echo sprintf(
					"<input type=\"hidden\" name=\"%s\" id=\"%s\" value=\"%s\" style=\"%s\" class=\"%s\" />",
					$field_name,
					$field_name,
					htmlspecialchars($field_value),
					implode( ';', $input_styles ),
					implode( ' ', $input_classes )
				);
				echo sprintf(
					"<input type=\"text\" name=\"%s_display\" id=\"%s_display\" data-target=\"%s\" value=\"%s\" style=\"%s\" class=\"%s time_display\" />",
					$field_name,
					$field_name,
					$field_name,
					htmlspecialchars($field_value_disp),
					implode( ';', $input_styles ),
					implode( ' ', $input_classes)
				);
				echo ' <i>hh:mm</i>';
				if ( ! $time_output ) {
					...
					<script type="text/javascript">
						jQuery( document ).ready( function($) {
							$( 'input.slt-cf-time.time_display' ).timePicker({
								step: 15,
								show24Hours: false
							});

							// experiment:
							$('input.slt-cf-time.time_display').change(function(){
								var dObj = $.timePicker(this).getTime();
								var ticks = (dObj.getTime() / 1000) - (dObj.getTimezoneOffset() * 60);

								// ticks represents our clock tick (sec.) since midnight
								$("#" + $(this).data("target")).val(ticks);
							});
						});
					</script>
					<?php
			...
			case 'date': {
				...
				*****************************************************************/
				// Label
				$field_name_display = sprintf("%s_display", $field_name);
				echo $before_label . '<label for="' . $field_name .'" class="' . implode( ' ', $label_classes ) . '">' . $field['label'] . '</label>' . $after_label;
				// Input
				$input_classes[] = 'slt-cf-date';
				echo $before_input;
				echo '<input type="text" name="' . $field_name_display . '" id="' . $field_name_display . '" style="' . implode( ';', $input_styles ) . '" class="' . implode( ' ', $input_classes ) . '" value="' . htmlspecialchars ( fh_format_jquery_date($field_value, $field['datepicker_format']) ) . '" />';
				echo '<input type="hidden" name="' . $field_name . '" id="' . $field_name . '" value="' . htmlspecialchars( $field_value ) . '" />';
				echo ' <i>' . str_replace( "y", "yy", $field['datepicker_format'] ) . '</i>';

				// The following is set once per field.  This is in contrast to doing it for the entire input class,
				// as we now need to handle each one specially using the altField/altFormat mechanism.
				?>
				<script type="text/javascript">
					jQuery( document ).ready( function($) {
						$( 'input#<?php echo $field_name_display; ?>' ).datepicker({
							dateFormat: '<?php echo $field['datepicker_format']; ?>',
							altField: '#<?php echo $field_name; ?>',
							altFormat: '@'
						});
					});
				</script>
