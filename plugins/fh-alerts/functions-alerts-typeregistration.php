...
// as needed.
function fh_alerts_register_capabilities( $post_type_name )
{
	global $wp_roles;
	...
	$ptype        = get_post_type_object($post_type_name);

	if( $ptype->map_meta_cap == 1 )
	{
...
add_action( 'fh_events_plugin_dependency_error', 'fh_admin_notice_dependency', 10, 2 );

function fh_alerts_admin_notice_dependency( $type, $plugin )
{
	eval("\$x = function() {
		?>
